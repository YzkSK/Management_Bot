...
- **mainブランチ**: リポジトリのデフォルトブランチ名は`main`。
...
- DB依存テストはテスト用PostgreSQLとマイグレーション済みスキーマを前提とする。CI(`.github/workflows/ci.yml`)と同様、ローカルでも`DATABASE_URL`を設定しマイグレーション適用後に実行する。DB用コンテナは一時的なテスト用コンテナ(実行のたびに作成し、テスト後は破棄する)として用意する。永続的なローカルDBに接続して実行しない。
