...
describe("correlateAuditLogEntry (実DB)", () => {
  ...
  test("IntegrationCreateはexecutorNameもintegrationカテゴリの行に保存する", async () => {
    const entry: AuditLogEntryInfo = {
      id: randomUUID(),
      guildId,
      action: "IntegrationCreate",
    ...
    const [row] = await db
      .select()
      .from(logEntries)
      .where(and(eq(logEntries.category, "integration"), sql`${logEntries.payload} ->> 'integrationId' = 'integration-2'`));
    expect(row?.payload).toMatchObject({ executorName: "モデレーター太郎" });
  });
